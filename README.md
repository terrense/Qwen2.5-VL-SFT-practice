...
## 0. Full Chain Runbook

如果你是第一次拿到这个目录，想从 0 到 1 把整条链路完整走一遍，直接先看：

- `FULL_CHAIN_RUNBOOK_0323.md`

这份文档是专门给“只有这个项目目录的人”准备的执行手册，内容包括：

- 拿到目录后的首次检查
- 环境安装顺序
- 哪些步骤是必跑，哪些是可选
- 从 `baseline -> train -> eval -> inference -> merge -> prune -> quantize -> trt -> deploy` 的完整命令顺序
- 每一步的输入、输出目录、成功标志和常见报错
